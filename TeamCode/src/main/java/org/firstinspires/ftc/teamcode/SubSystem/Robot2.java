...
public class Robot2 {
    ...
    public void init(HardwareMap hw) {
        ...
        Pose savedPose = Auto_Pose.load();

        if (savedPose != null) {
            follower.setStartingPose(savedPose);
            telemetry.addLine("Pose loaded from Auto");
            telemetry.addData("x", savedPose.getX());
            telemetry.addData("y", savedPose.getY());
            telemetry.addData("h deg", Math.toDegrees(savedPose.getHeading()));
        } else {
            telemetry.addLine("No saved pose found");
        }
